...
void tcp_init_sock(struct sock *sk)
{
	 ...
	 */
	sk->sk_sndbuf = READ_ONCE(sysctl_tcp_wmem[1]);
	sk->sk_rcvbuf = READ_ONCE(sysctl_tcp_rmem[1]);
...
unsigned int tcp_poll(struct file *file, struct socket *sock, poll_table *wait)
{
	...
	if (shutdown == SHUTDOWN_MASK || sk->sk_state == TCP_CLOSE)
		mask |= POLLHUP;
		...
		int optname, char __user *optval, int __user *optlen)
{
	...
	case TCP_LINGER2:
		...
		if (val >= 0)
			val = (val ? : READ_ONCE(sysctl_tcp_fin_timeout)) / HZ;
		break;
	case TCP_DEFER_ACCEPT:
...
restart:
		...
		sk_nulls_for_each(sk, node, &tcp_hashinfo.ehash[bucket].chain) {
			...
			if (!sock_flag(sk, SOCK_DEAD)) {
				...
				WRITE_ONCE(sk->sk_err, ETIMEDOUT);
				sk->sk_error_report(sk);
				tcp_done(sk);
			}
