...
static int msm_route_ec_ref_rx = 13; /* NONE */
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_debug("%s: shift 0x%x rshift 0x%x val %ld\n", __func__, mc->shift,
		mc->rshift, ucontrol->value.integer.value[0]);
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	for (be_idx = 0; be_idx < MSM_BACKEND_DAI_MAX; be_idx++) {
		if(msm_bedais[be_idx].active) {
			if (afe_get_port_type(msm_bedais[be_idx].port_id) == MSM_AFE_PORT_TYPE_TX)
				break;
		}
	}	
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	for (be_idx = 0; be_idx < MSM_BACKEND_DAI_MAX; be_idx++) {
		if(msm_bedais[be_idx].active) {
			if (afe_get_port_type(msm_bedais[be_idx].port_id) ==
				MSM_AFE_PORT_TYPE_RX)
				break;
		}
	}
	...
	if ((be_idx < MSM_BACKEND_DAI_MAX) &&
		msm_bedais[be_idx].active) {
		ret = adm_set_sound_booster(msm_bedais[be_idx].port_id,
...
static const char *const ec_ref_rx[] = { "None", "SLIM_RX", "I2S_RX",
	...
	"SLIM_5_RX", "SLIM_1_TX", "QUIN_MI2S_TX", "QUAT_MI2S_RX"};
